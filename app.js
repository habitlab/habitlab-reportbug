...
(function(){
  var koa, koaRouter, koaLogger, koaBodyparser, koaJsonp, koaStatic, sendgrid, jsyaml, kapp, app, helper, getsecret, sg, default_from_email, default_to_email, cloudinary, Gitter, gitter, ref$, add_noerr, cfy, co, img_data_base64, upload_to_cloudinary, port;
  koa = require('koa');
  ...
  app.post('/report_bug', function*(){
    var ref$, message, screenshot, other, screenshot_url, err, gitter_message;
    this.type = 'json';
    ...
    this.body = gitter_message;
    return 'room = yield gitter.rooms.join(\'habitlab/habitlab\')\nroom.send("""\nHere is a long message.\nAnd more.\nMultiline supported right?\n\n[#{img_url}](#{img_url})\n""")';
  });
  app.get('/send_test_gitter', function*(){
    var img_url, room;
    this.type = 'json';
    img_url = (yield upload_to_cloudinary(img_data_base64));
    room = (yield gitter.rooms.join('habitlab/habitlab'));
    room.send("Here is a long message.\nAnd more.\nMultiline supported right?\n\n[" + img_url + "](" + img_url + ")");
    return this.body = img_url;
  });
  app.get('/send_test_email', function*(){
    var from_email, to_email, subject, content, mail, attachment, request, response;
    this.type = 'json';
    ...
    response = (yield function(it){
      return sg.API(request, it);
    });
    return this.body = JSON.stringify(response);
  });
  kapp.use(app.routes());
